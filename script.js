document.addEventListener('DOMContentLoaded', () => {
    ...
    const CONFIG = {
        ...
        EMA_ANALYSIS: { SYMBOLS: ['BTC-USDT', 'ETH-USDT'], TIMEFRAMES: ['1hour', '2hour', '4hour', 'daily'], PERIODS: [13, 49, 100, 200, 500, 1000], SHORT_NAMES: {'15min': '15m', '30min': '30m', '1hour': '1H', '2hour': '2H', '4hour': '4H', 'daily': '1D', 'weekly': '1W', 'monthly': '1M'} }
    };
   ...
   async function fetchAndRenderMarketSummary() {
    ...
    function renderOrUpdateChart(canvasId, type, chartData, titleText, scalesConfig = {}, pluginsConfig = {}) {
        ...
        const chartOptions = {
            responsive: true,
            maintainAspectRatio: false, // This is key for mobile sizing
            plugins: {
                title: { display: true, text: titleText, font: { size: 18 } },
                ...pluginsConfig
            },
            scales: {
                x: { type: 'time', time: { unit: 'hour', tooltipFormat: 'MMM dd, yyyy HH:mm', displayFormats: { hour: 'HH:mm' } }, title: { display: true, text: 'Time (UTC)' } },
                ...scalesConfig
            }
        };

        // Check if a chart instance already exists in our persistent state object
        if (STATE.chartInstances[canvasId]) {
            // If it exists, just update its data and options, then call update()
            const chart = STATE.chartInstances[canvasId];
            chart.data = chartData;
            chart.options = chartOptions;
            chart.update();
        } else {
            // If it doesn't exist, create it for the first time and store it
  ...
  async function fetchAndRenderHistoricalCharts() {
    ...
    function renderCompletedTable(data) {
        ...
        const rowsHtml = Object.keys(trades).sort((a, b) => new Date(trades[b].closed_date) - new Date(trades[a].closed_date)).map(id => {
            ...
            if (trade.closed_date) { const closedDate = new Date(trade.closed_date); closedPerthDateStr = getPerthDateString(closedDate); isHistorical = (closedPerthDateStr !== perthTodayStr); } else { isHistorical = true; }
    ...
    function renderGenericImageLibrary(imagesData, containerElement, libraryType) {
        ...
        const imagesHtml = Object.keys(images).sort((a, b) => images[b].timestamp - images[a].timestamp).map(id => {
            ...
            if (libraryType === 'setups_images' && imgData.timestamp) {
                ...
                if (isHistorical) { wrapperClasses += ' historical-entry'; wrapperStyle = 'style="display: none;"'; }
            }
        ...
        if (libraryType === 'setups_images') { updateToggleButtonsVisibility(); }
        adjustCollapsibleSize(containerElement);
    }

    function renderChartLibrary2Folders() {
        if (!DOM.chartLibrary2Container) return;
        const foldersHtml = CL2_FOLDERS.map(folder => { const imageCount = STATE.chartLibrary2Data[folder] ? Object.keys(STATE.chartLibrary2Data[folder]).length : 0; return `<div class="cl2-folder-card" data-folder="${folder}"><div class="cl2-folder-icon">📁</div><h4>${folder}</h4><div class="cl2-folder-count">${imageCount} image${imageCount !== 1 ? 's' : ''}</div></div>`; }).join('');
        ...
        if (STATE.isMasterUser) {
            document.querySelectorAll('.cl2-folder-card').forEach(folderEl => {
                ...
                folderEl.addEventListener('drop', e => { folderEl.classList.remove('drag-over'); if (e.dataTransfer?.files.length > 0) handleChartLibrary2Upload(e.dataTransfer.files, folderEl.dataset.folder); });
            });
        }
    }
    ...
    function renderEmaSnapshot(safeSymbol) {
        ...
        if (currentPrice > 0) {
            ...
            htfForSummary.forEach(tf => {
                ...
                if (tfData) { PERIODS.forEach(period => ['EMA', 'SMA'].forEach(type => { const val = tfData[`${type}_${period}`]; if (val !== undefined) { levelsHtf.push({ label: `${SHORT_NAMES[tf]} ${type} ${period}`, value: val, isSupport: val < currentPrice }); } })); }
            });
    ...
    function renderSetupsTable(tableElement, data, notesTextareaId) {
        ...
        if (notesTextarea) { notesTextarea.value = data.global_notes || ''; adjustCollapsibleSize(notesTextarea); }
    ...
    const savePrimarySetupsNotes = () => { if (!STATE.isMasterUser) return; saveSetupsTable(DOM.setupsTable1, dbRefs.setups1, DOM.setupsSaveStatus1, 'primary-setups-notes'); };
    const saveSecondarySetupsNotes = () => { if (!STATE.isMasterUser) return; saveSetupsTable(DOM.setupsTable2, dbRefs.setups2, DOM.setupsSaveStatus2, 'secondary-setups-notes'); };
    ...
    // --- EVENT LISTENERS ---
     function setupEventListeners() {
        ...
        DOM.primarySetupsNotes.addEventListener('input', (e) => { autoResizeTextarea(e.target); debouncedResizeParent(e.target); clearTimeout(STATE.setups1NotesTimer); STATE.setups1NotesTimer = setTimeout(savePrimarySetupsNotes, 750); });
        DOM.secondarySetupsNotes.addEventListener('input', (e) => { autoResizeTextarea(e.target); debouncedResizeParent(e.target); clearTimeout(STATE.setups2NotesTimer); STATE.setups2NotesTimer = setTimeout(saveSecondarySetupsNotes, 750); });
    ...
    function calculateTradeMetrics(tradeData) {
        const entry = parseFloat(tradeData.entry); const stoploss = parseFloat(tradeData.stoploss); const target = parseFloat(tradeData.target); const size = parseFloat(tradeData.size); const type = tradeData.type;
        ...
        const expected_profit_percent = (expected_profit / size) * 100; const expected_loss_percent = (expected_loss / size) * 100;
        return { rr: rr.toFixed(2), estimated_total_fee: estimated_total_fee, expected_profit: expected_profit, expected_loss: expected_loss, expected_profit_percent: expected_profit_percent, expected_loss_percent: expected_loss_percent };
    }

    function handleAddTradeSubmit(e) {
        e.preventDefault(); if (!STATE.isMasterUser) return;
        const trade = { symbol: document.getElementById('trade-symbol').value, type: document.getElementById('trade-type').value, entry: parseFloat(document.getElementById('entry-price').value), stoploss: parseFloat(document.getElementById('stop-loss').value), target: parseFloat(document.getElementById('target').value), size: parseFloat(document.getElementById('position-value').value), date: firebase.database.ServerValue.TIMESTAMP, entry_prices: [] };
    ...
    function handleSentimentLog(e) { const button = e.target.closest('.sentiment-buttons button'); if (!button) return; const { asset, sentiment } = button.dataset; if (!asset || !sentiment) return; dbRefs.sentimentReceipts.push({ asset, sentiment, timestamp: firebase.database.ServerValue.TIMESTAMP, viewerId: getViewerId() }); }
    function handleSentimentAction(e) { if (!STATE.isMasterUser) return; const editBtn = e.target.closest('.edit-sentiment-note-btn'); if (editBtn) handleSentimentNoteEdit(editBtn); const deleteBtn = e.target.closest('.delete-sentiment-btn'); if (deleteBtn) handleSentimentDelete(deleteBtn); }
    ...
    function handleSentimentNoteEdit(button) { const receiptId = button.dataset.id; if (!receiptId) return; const receiptRef = dbRefs.sentimentReceipts.child(receiptId); receiptRef.child('note').once('value', snapshot => { const newNote = prompt('Enter note:', snapshot.val() || ""); if (newNote !== null) receiptRef.update({ note: newNote }); }); }
    ...
    function handleAddStaggeredEntry(tradeId) {
        ...
        tradeRef.once('value', snapshot => {
            ...
            const newMetrics = calculateTradeMetrics(tempTradeForCalc);
            const updatedTrade = { ...currentTrade, entry: newAverageEntryPrice, size: totalValue, entry_prices: updatedEntryPrices, ...newMetrics, note: (currentTrade.note || '') + `\n+ Added $${newPositionValue.toFixed(2)} @ $${newEntryPrice.toFixed(4)} on ${new Date().toLocaleString()}` };
    ...
    function handleLibraryActions(e, libraryType, dbRef) {
        ...
        if (deleteBtn && STATE.isMasterUser) {
            ...
            if (confirm(`Delete this image?\n\n${filename}`)) { const storagePath = `${libraryType}/${filename}`; storage.ref(storagePath).delete().then(() => dbRef.child(id).remove()).catch((err) => { if (err.code === 'storage/object-not-found') dbRef.child(id).remove(); else alert('Error deleting image.'); }); }
        } else if (editBtn && STATE.isMasterUser) {
            const { id } = editBtn.dataset;
            const imageRef = dbRef.child(id);
            imageRef.child('caption').once('value', snapshot => { const newCaption = prompt('Enter image caption:', snapshot.val() || ""); if (newCaption !== null) imageRef.update({ caption: newCaption.trim() }); });
        }
    }
    ...
    function handleChartLibrary2Click(e) {
        ...
        if (deleteBtn && STATE.isMasterUser) {
            ...
            if (confirm(`Delete this image from ${folder}?\n\n${filename}`)) { const storagePath = `chart_library_2/${folder}/${filename}`; storage.ref(storagePath).delete().then(() => dbRefs.chartLibrary2.child(folder).child(id).remove()).catch((err) => { if (err.code === 'storage/object-not-found') dbRefs.chartLibrary2.child(folder).child(id).remove(); else alert('Error deleting image.'); }); }
        }
    }
    ...
    function handleTradeAction(e) {
        ...
        if (STATE.isMerging) {
            ...
            if (targetRow && targetRow.parentElement.id === 'completed-trades-tbody' && !e.target.closest('button')) { const absorbedTradeId = targetRow.querySelector('.action-buttons button')?.dataset.id; if (absorbedTradeId) executeMerge(STATE.mergeBaseTradeId, absorbedTradeId); }
            return;
        }
        ...
        else if (button.matches('.note-btn, .note-btn-completed')) { tradeRef.child('note').once('value', snapshot => { const newNote = prompt("Enter note:", snapshot.val() || ""); if (newNote !== null) tradeRef.update({ note: newNote }); }); }
    }
    ...
    function subscribeToData() {
        ...
        dbRefs.persistentNotes.on('value', (s) => { STATE.persistentNotes = s.val() || ''; if (document.activeElement !== DOM.persistentNotesEl) { DOM.persistentNotesEl.value = STATE.persistentNotes; } adjustCollapsibleSize(DOM.persistentNotesEl); });
