...
### REFACTORED: The efficient way to get all option data ###
...
# Core Calculations
# -----------------------------
### MODIFIED to accept summary data directly ###
def aggregate_market_data(all_summaries: List[Dict[str, Any]], end_date: datetime):
    ...
    for summary in all_summaries:
        ...
        # Greeks are now top-level keys in the summary object
        gamma = float(summary.get('gamma', 0.0)); delta = float(summary.get('delta', 0.0)); vega = float(summary.get('vega', 0.0)); theta = float(summary.get('theta', 0.0))
        ...
        grouped_options[expiry_dt].append({
            ...
            'volume_24h': float(summary.get('volume', 0.0)), # volume is a top-level key
            'dealer_gamma_contrib': dealer_gamma_contrib, 'dealer_delta_contrib': dealer_delta_contrib, 
            'dealer_vega_contrib': dealer_vega_contrib, 'dealer_theta_contrib': dealer_theta_contrib
        })
...
def find_oi_walls(options_list: List[Dict[str, Any]], top_n: int) -> Dict[str, List[Dict[str, Any]]]:
    ...
    # Using 'open_interest' as the key for clarity in the JSON
...
# -----------------------------
# Final JSON Assembly (Unchanged logic, just parameter name changes)
# -----------------------------
def process_and_save_data(currency: str, output_filename: str,
    ...
    market_summary = {"total_open_interest": round(total_oi, 2), "total_notional_oi_usd": round(total_oi * spot_price, 2), "total_volume_24h": round(total_volume_24h, 2), "pcr_by_open_interest": round(pcr_by_oi, 4), "total_dealer_gamma_exposure": round(total_gex, 4), "total_dealer_delta_exposure": round(total_dex, 2), "total_dealer_vega_exposure_usd": round(total_vex, 2), "total_dealer_theta_exposure_usd": round(total_thex, 2), "gamma_flip_level_usd": gamma_flip_level, "cumulative_flow_since_utc_midnight": cumulative_flow}
...
if __name__ == "__main__":
    for currency in TARGET_CURRENCIES:
        ...
        if not spot_price: logging.error(f"Fatal: Could not fetch {currency} index price. Skipping."); continue

        flow_data = get_cumulative_flow(currency)

        ### REFACTORED to be efficient ###
